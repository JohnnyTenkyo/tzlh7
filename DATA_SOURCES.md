...
建议先不再盲目新增密钥，而是将当前策略改为“本地缓存优先、只增量更新过期标的、低频扫描”。如需增加一把免费密钥，优先接入 Twelve Data，并将其用于精选清单或小时/15 分钟回退。若要稳定运行约 793 只股票的每日全量扫描，建议后续实现 Alpaca K 线适配器，或选定一个具备足够日线授权的单一主数据源。
